...
class ComplianceAgent(BaseAgent):
    ...
    async def run(self, case_id: str) -> AgentResult:
        ...
        try:
            ...
            all_required = await async_gremlin_submit(
            ...
            missing_components = await async_gremlin_submit(
                "g.V().has('Case', 'case_id', cid)"
            ...
            logger.info(
                f"[Compliance] Found {len(missing_components)} missing components"
            )

            # ── Step 5: Fetch legal basis from KG ────────────────────
            legal_basis = await async_gremlin_submit(
                "g.V().has('TTHCSpec', 'code', code)"
                ".out('GOVERNED_BY').valueMap(true)",
                {"code": tthc_code},
            )
            ...
            case_context = {
                ...
                "tthc_name": tthc_name,
                "documents_submitted": [
                    {"type": d["type"], "entities": d["entities"][:5]}
            ...
            for gap in confirmed_gaps:
                ...
                # Write Gap vertex + HAS_GAP edge via template
                ...
                # Write GAP_FOR edge to RequiredComponent if _kg_id available
                ...
                if req_kg_id:
                    try:
                        await async_gremlin_submit(
                            "g.V().has('gap_id', gid)"
                            ".addE('GAP_FOR')"
                            ".to(g.V().has('_kg_id', rid))",
                            {"gid": gap_id, "rid": req_kg_id},
                        )
                    except Exception as e:
                        logger.warning(f"[Compliance] Failed to write GAP_FOR edge: {e}")

                # Write Citation vertex + CITES edge if law_citation present
                ...
                if law_citation:
                    ...
                    try:
                        ...
                        # Link Gap -> CITES -> Citation
                        await async_gremlin_submit(
                            "g.V().has('gap_id', gid)"
                            ".addE('CITES')"
                            ".to(g.V().has('citation_id', cid))",
                            {"gid": gap_id, "cid": cit_id},
                        )
                    except Exception as e:
                        logger.warning(f"[Compliance] Failed to write Citation: {e}")
            ...
            total_mandatory = sum(
                1 for c in required_components_data if c["is_required"]
            )
            blocker_count = sum(
                1 for g in confirmed_gaps if g.get("severity") == "blocker"
            )
            satisfied_count = total_mandatory - blocker_count
            compliance_score = round(
                (satisfied_count / max(total_mandatory, 1)) * 100, 1
            )

            # ── Step 10: Write compliance_score to Case ──────────────
            await async_gremlin_submit(
                "g.V().has('Case', 'case_id', cid)"
                ".property('compliance_score', score)",
    ...
    async def _evaluate_compliance(self, case_context: dict) -> dict[str, Any]:
        ...
        for attempt in range(2):
            ...
            try:
                ...
                if attempt == 0:
                    logger.warning(
                        "[Compliance] Invalid JSON from Qwen, retrying"
                    )
                    ...
                    messages.append({
                        ...
                        "content": (
                            ...
                            '"severity": "blocker|warning|info", "fix_suggestion": "...", '
                    ...
                    logger.error(
                        f"[Compliance] JSON parse failed after retry: {content[:200]}"
                    )
                    return {
                        "gaps": [
                            {
