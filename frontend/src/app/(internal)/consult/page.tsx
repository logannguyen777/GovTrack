...
export default function ConsultPage() {
  ...
  async function handleAiDraft() {
    ...
    setAiDrafting(true);
    try {
      // Simulate Drafter agent pre-fill. Real impl can POST to
      // /api/agents/run/{case_id} with pipeline=drafter and read the artifact.
      ...
      const draft =
        stance === "agree"
          ? `Sau khi nghiên cứu hồ sơ ${selected.case_code} và các căn cứ pháp lý liên quan, ` +
            ...
            `Hồ sơ đã đáp ứng các yêu cầu theo quy định hiện hành. Đề nghị tiến hành các bước tiếp theo.`
          : stance === "disagree"
            ? `Qua rà soát hồ sơ ${selected.case_code}, Phòng Pháp chế nhận thấy chưa đủ cơ sở để chấp thuận. ` +
              `Đề nghị đơn vị chủ trì bổ sung tài liệu chứng minh và làm rõ phần: "${selected.main_question}".`
            : `Nội dung xin ý kiến ${selected.case_code} vượt ngoài phạm vi tham mưu của Phòng Pháp chế. ` +
              `Đề nghị chuyển đến cơ quan chuyên môn có thẩm quyền.`;
      setOpinion(draft);
      toast.success("AI đã soạn thảo ý kiến — xin vui lòng rà soát trước khi gửi");
    } finally {
      setAiDrafting(false);
    }
  }
